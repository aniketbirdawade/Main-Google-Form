...
.container { max-width: 700px; margin: 50px auto; background: #fff; padding: 25px; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.1); }
h2 { text-align: center; color: #333; }
select, input[type=submit] { padding: 8px; margin-top: 10px; border-radius: 4px; border: 1px solid #ccc; }
...
if(conn != null){
    try {
        // Step 1: Select form if form_id not submitted
...
<%
            while(rsForms.next()){
%>
<option value="<%= rsForms.getInt("form_id") %>"><%= rsForms.getString("title") %> (ID: <%= rsForms.getInt("form_id") %>)</option>
<%
            }
%>
</select>
...
<%
            ...
            if(submitFormIdStr != null){
                ...
                PreparedStatement psResp = conn.prepareStatement(
                    "INSERT INTO responses(form_id) VALUES(?)", Statement.RETURN_GENERATED_KEYS
                );
                ...
                PreparedStatement psQ = conn.prepareStatement(
                    "SELECT question_id FROM questions WHERE form_id=?"
                );
                psQ.setInt(1, formId);
                ResultSet rsQ = psQ.executeQuery();
                while(rsQ.next()){
                    ...
                    if(ans != null){
                        PreparedStatement psDetail = conn.prepareStatement(
                            "INSERT INTO response_details(response_id, question_id, answer) VALUES(?, ?, ?)"
                        );
